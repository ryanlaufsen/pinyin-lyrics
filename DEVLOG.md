...
- Corrected Traditional-only Chinese glyph rendering after the follow-up clarification that the breakage was font coverage, not script-conversion direction.
- Added Traditional Chinese font families to the static reader CDN preload set: `Noto Sans TC`, `Noto Sans HK`, `Noto Serif TC`, `Noto Serif HK`, and `LXGW WenKai TC`.
...
- Commit `e1d3cd4` loaded real CJK character-style fonts, removed fake Round shadows, added script-role font overrides, expanded e2e font assertions, and recorded the FE-015 handoff.
