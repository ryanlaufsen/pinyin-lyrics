...
| Static lyric editing/ad layout                | Automated         | UI Design Manager        | `static-bafang.spec.ts` covers desktop half-width lyrics/custom-input column plus ad space, mobile stacking, ad placeholder presence, and label spacing. |
...
| Control names, roles, and states         | Automated         | QA/Accessibility Manager | Static route asserts theme segmented control, guide toggle role/name/state, script buttons, and sizing controls in `static-bafang.spec.ts`; broader workspace route contract remains under QA-002. |
...
| Dark/OLED contrast                       | Automated         | UI Design Manager        | Static route asserts Dark/OLED pinyin/Hanzi contrast plus panel, input, and ad-space text and component-boundary contrast metrics.   |
