...
- Static Chinese character styles now include Traditional-capable font families and CSS routing: `Noto Sans TC/HK`, `Noto Serif TC/HK`, and `LXGW WenKai TC` are loaded, `繁` mode prefers TC/HK stacks, and source-mode Traditional-only glyphs are marked so they do not fall back to Simplified-oriented Chinese fonts.
